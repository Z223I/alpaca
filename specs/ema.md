...
Use all of this code including __main__.

```python
import pandas as pd
import numpy as np

def calculate_ema(df, price_column='close', period=9):
    """
...
def calculate_ema_manual(df, price_column='close', period=9):
    """
    Manual calculation of EMA using the traditional formula for candlestick data.
...
if __name__ == "__main__":
    ...
    if success:
        ...
        for i, (idx, row) in enumerate(df.iterrows()):
            if row['ema_9'] > 0:
                print(f"Day {i+1} ({idx.strftime('%Y-%m-%d')}): Close=${row['close']:.2f}, EMA=${row['ema_9']:.2f}")
            else:
                print(f"Day {i+1} ({idx.strftime('%Y-%m-%d')}): Close=${row['close']:.2f}, EMA=0.00 (insufficient data)")
    else:
    ...
    success, ema_manual_result = calculate_ema_manual(df, price_column='close', period=9)
    ...
    if success:
        ...
        for i, (idx, row) in enumerate(df.iterrows()):
            if row['ema_9_manual'] > 0:
                print(f"Day {i+1} ({idx.strftime('%Y-%m-%d')}): Close=${row['close']:.2f}, EMA=${row['ema_9_manual']:.2f}")
            else:
                print(f"Day {i+1} ({idx.strftime('%Y-%m-%d')}): Close=${row['close']:.2f}, EMA=0.00 (insufficient data)")
    else:
    ...
    success, ema_high_result = calculate_ema(df, price_column='high', period=9)
    ...
    if success:
        ...
        for i, (idx, row) in enumerate(df.iterrows()):
            if row['ema_9_high'] > 0:
                print(f"Day {i+1} ({idx.strftime('%Y-%m-%d')}): High=${row['high']:.2f}, EMA=${row['ema_9_high']:.2f}")
            else:
                print(f"Day {i+1} ({idx.strftime('%Y-%m-%d')}): High=${row['high']:.2f}, EMA=0.00 (insufficient data)")
    else:
    ...
    success, ema_fail = calculate_ema(small_df, price_column='close', period=9)
    ...
    success, ema_fail2 = calculate_ema(df, price_column='invalid_column', period=9)
    ...
    print(f"where α (smoothing factor) = 2 / (period + 1) = 2 / (9 + 1) = 0.2")
    print("\nSupported price columns: open, high, low, close, or any numeric column in the DataFrame")
    print("Returns: (success: bool, ema: pandas Series) - EMA initialized to zeros on failure")
```
