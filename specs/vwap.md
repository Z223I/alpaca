...
def calculate_vwap(df, price_column='close'):
    """
...
def calculate_vwap_typical(df):
    ...
    Args:
        df: DataFrame containing candlestick data (must have high, low, close, volume)
...
def calculate_vwap_ohlc(df):
    """
    Calculate VWAP using OHLC average price for candlestick data.
    
    Args:
        df: DataFrame containing candlestick data (must have open, high, low, close, volume)
...
if __name__ == "__main__":
    ...
    if success:
        ...
        for i, (idx, row) in enumerate(df.iterrows()):
            print(f"Day {i+1} ({idx.strftime('%Y-%m-%d')}): Close=${row['close']:.2f}, Volume={row['volume']}, VWAP=${row['vwap_close']:.2f}")
    else:
    ...
    if success:
        ...
        for i, (idx, row) in enumerate(df.iterrows()):
            ...
            print(f"Day {i+1} ({idx.strftime('%Y-%m-%d')}): Typical=${typical:.2f}, Volume={row['volume']}, VWAP=${row['vwap_typical']:.2f}")
    else:
    ...
    if success:
        ...
        for i, (idx, row) in enumerate(df.iterrows()):
            ...
            print(f"Day {i+1} ({idx.strftime('%Y-%m-%d')}): OHLC=${ohlc:.2f}, Volume={row['volume']}, VWAP=${row['vwap_ohlc']:.2f}")
    else:
    ...
    print("- calculate_vwap_typical(): Uses typical price (High + Low + Close) / 3")
    print("- calculate_vwap_ohlc(): Uses OHLC average (Open + High + Low + Close) / 4")
    print("Returns: (success: bool, vwap: pandas Series) - VWAP initialized to zeros on failure")
