"""
Display top market movers (gainers and losers) to console.

This atom provides formatted output for top gainers and losers
...
def print_top_movers(api_client: Any, market_type: str = 'stocks',
                     account_name: str = None, environment: str = None) -> bool:
    """
    Display top market gainers and losers to console.
    ...
    data = get_top_movers(api_client, market_type, account_name, environment)
    ...
    print("\n" + "=" * 80)
    print(f"🔴 TOP LOSERS ({market_type.upper()})")
    print("=" * 80)

    if losers:
        print(f"{'Rank':<6} {'Symbol':<10} {'Price':<12} {'Change':<12} {'% Change':<12}")
        print("-" * 80)
        for idx, mover in enumerate(losers, 1):
            symbol = mover.get('symbol', 'N/A')
            price = mover.get('price', 0)
            change = mover.get('change', 0)
            percent_change = mover.get('percent_change', 0)
